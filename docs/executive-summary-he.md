...
- עלויות ההגנה הנוכחיות **אינן בנות קיימא**: טיל Patriot MSE ב-$4–5M מול מל"ט ב-$20–50K (יחס: 150:1–300:1)
...
## רשת CF מול Iron Beam, Trophy APS ולייזרים

![CF Net מול הגנה אקטיבית](../images/infographics/cf-net-vs-active-defense.png)

Rafael סיפקה את Iron Beam (לייזר 100kW, דצמבר 2025) ושדרגה את Trophy APS להגנה נגד מל"טים. מערכות מרשימות אך עם פערים קריטיים:

| גורם | Iron Beam | Trophy APS | רשת CF |
|-------|-----------|------------|--------|
| עלות/התערבות | $5-10 אנרגיה (מערכת $50-100M) | אפקטור נצרך | **$0** |
| עובד בערפל/גשם? | **לא** | כן | **כן** |
| הגנת נחיל (50+) | 12/דקה מקסימום | 1 בכל פעם | **כולם בו-זמנית** |
| FPV סיב אופטי? | צריך לזהות קודם | טווח קרוב בלבד (<50 מ') | **כן (פסיבי)** |
| פסיבי 24/7? | לא | לא | **כן** |

**לא רשתות או לייזרים — רשתות וגם לייזרים.** רשתות CF הן השכבה הפסיבית שתופסת מה שמערכות אקטיביות מפספסות.

![ארכיטקטורת הגנה שכבתית](../images/infographics/layered-defense-architecture.png)
...
**הקשר**: תקציב הביטחון של ישראל >$45B/שנה (2026). כיפת ברזל עלתה ~$3B. לילה אחד של הגנת Patriot עולה $200M+.
...
### פתרון ביניים (פריסה תוך 6 חודשים)

פריסת 10–40 אווירוסטטים טקטיים (Skystar 180/DURUS, כבר בשירות צה"ל) עם תליוני כבל CF בנקודות חנק גבוליות.
...
| השקעה | $3–10M למגזר |
| זמן פריסה | 30 דקות לאווירוסטט |
| ניידות טקטית | מבוסס נגרר, נע עם כוחות קרקע |

פועל גם טקטית: הגנת היקף בסיס (30 דקות הקמה), ליווי שיירות (שיטת דילוג), הגנת אזור כינוס. אותו ציוד משרת משימות ניידות, חצי-סטטיות וסטטיות.

---

*מסמך מחקר מלא (60+ עמודים) זמין לפי בקשה.*  
